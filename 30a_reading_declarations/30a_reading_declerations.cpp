...
2. going right, because we can:    foo is "array of 7 long
3. going left, because we must:		foo is array of 7 "pointer to"...long
4. going left, because we must:		foo is array of 7 pointers to long pointers.
in other words, foo is an array of 7 pointers to long pointers (i.e each long pointer points to a long pointer.
the latter pointer can point to a single long, or a consequtive longs.
...
/*
Note: the rhs here is NOT A DECLARATION , but it is an expression that is evaluated into a single value:
 ...
 Aside from that,  we can use the same rules for navigation:
...
2. go right when you can, go left when you must
 ...
 4 to a pointer to an int) and then this address is dereferenced, casted to an int pointer
