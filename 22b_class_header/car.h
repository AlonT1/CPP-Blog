...
Notes:
1. User defined types (such as classes) are exempt from ODR (one defintion rule - 1
definition across all the program). multiple different source files (cpp) can potentially include 
a header file that has fully defined variables and functions. This inclusion will lead 
each cpp file to have a class with fully defined functions during compilation. In other words, a single
cpp cannot have multiple redifnitions of the same class, but the same class can be defined in multiple cpp's
(the definitions can be identical or different across files).
...
Including fully defined header is problematic for 2 reasons:
	...
	all the files that make use of the class. instead, when using a header with forward declarations
	and a single defined cpp file,  we can simply recompile the cpp definition of the class
	(recompilation of a single translation unit), and relink to create exe.


Advantages for using header files:
